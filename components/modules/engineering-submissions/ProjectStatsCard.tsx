...
import { Building2, MapPin, Briefcase, LayoutGrid } from 'lucide-react';
        ...
        <CardContent className="pt-5 space-y-6">
          <div className="grid grid-cols-2 gap-4">
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              ...
              <span className="truncate font-medium text-foreground">{project.client_name}</span>
            </div>
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              <MapPin className="h-4 w-4 shrink-0" />
              <span className="truncate font-medium text-foreground">{project.location || 'N/A'}</span>
            </div>
          </div>
            ...
            <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2 scrollbar-thin">
              {Object.entries(STATUS_CATEGORIES).map(([key, label]) => (
                <ProgressCategoryRow 
                  key={key} 
                  label={label} 
                  stats={stats?.[key] || { submitted: 0, in_progress: 0, rejected: 0, approved: 0 }} 
                />
              ))}
            </div>
          </div>
